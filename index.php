...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
            ...
            if ($urun_id > 0 && $miktar > 0 && $personel_id > 0) {
                // Stok kontrolü hala önemli, bu kısmı koruyoruz.
                ...
                if ($current_stock >= $miktar) {
                    // Transaction başlatmaya gerek kalmadı çünkü tek bir INSERT işlemi yapıyoruz.
                    // Tetikleyici geri kalanı halledecek.
                    try {
                        $stmt = $conn->prepare("INSERT INTO Satış (PersonelID, ÜrünID, Miktar, BirimFiyat) VALUES (?, ?, ?, ?)"); 
                        $stmt->bind_param("iiid", $personel_id, $urun_id, $miktar, $birim_fiyat); 
                        if (!$stmt->execute()) { 
                            throw new Exception("Satış eklenirken hata: " . $stmt->error); 
                        } 
                        $stmt->close();
                        
                        // --- ÇIKARILAN KISIM ---
                        // $update_stock_stmt = $conn->prepare("UPDATE Ürün SET Stok = Stok - ? WHERE ÜrünID = ?"); 
                        // $update_stock_stmt->bind_param("ii", $miktar, $urun_id); 
                        // if (!$update_stock_stmt->execute()) { 
                        //     throw new Exception("Stok güncellenirken hata: " . $update_stock_stmt->error); 
                        // } 
                        // $update_stock_stmt->close();
                        // --- ÇIKARILAN KISIM BİTİŞ ---
                        
                        $message = ['type' => 'success', 'text' => 'Satış başarıyla gerçekleştirildi. Stok, veritabanı tarafından otomatik olarak güncellendi.'];
                    } catch (Exception $e) { 
                        $message = ['type' => 'danger', 'text' => 'HATA: ' . htmlspecialchars($e->getMessage())]; 
                    }
                } else { 
                    $message = ['type' => 'danger', 'text' => 'Yetersiz stok! Mevcut stok: ' . $current_stock]; 
                }
            } else { 
                $message = ['type' => 'danger', 'text' => 'Lütfen tüm satış alanlarını doldurun.']; 
            }
            break;
            ...
            if ($urun_id > 0 && $tedarikci_id > 0 && $miktar > 0 && $personel_id > 0) {
                 // Transaction başlatmaya gerek kalmadı, tek INSERT var.
                try {
                    ...
                    $stmt = $conn->prepare("INSERT INTO Alım (TedarikçiID, PersonelID, ÜrünID, Miktar, BirimFiyat) VALUES (?, ?, ?, ?, ?)"); 
                    $stmt->bind_param("iiiid", $tedarikci_id, $personel_id, $urun_id, $miktar, $birim_fiyat); 
                    if (!$stmt->execute()) { 
                        throw new Exception("Alım eklenirken hata: " . $stmt->error); 
                    } 
                    $stmt->close();
                    ...
                    $message = ['type' => 'success', 'text' => 'Alım işlemi başarıyla eklendi. Stok, veritabanı tarafından otomatik olarak güncellendi.'];
                } catch (Exception $e) { 
                    $message = ['type' => 'danger', 'text' => 'HATA: ' . htmlspecialchars($e->getMessage())]; 
                }
            } else { 
                $message = ['type' => 'danger', 'text' => 'Lütfen tüm alım alanlarını doldurun.']; 
            }
            break;
